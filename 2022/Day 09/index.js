...
const ropes = [[...Array(2)].map(() => [0, 0]), [...Array(10)].map(() => [0, 0])];
const tailsVisitedPositions = [[], []];

for (const line of input) {
    const directions = { "L": 0, "R": 0, "U": 0, "D": 0 };
    directions[line[0]] += line[1];
    const movements = [directions["R"] - directions["L"], directions["U"] - directions["D"]]; // x, y
    const activeMovementIdx = movements.findIndex((elem) => elem !== 0);
    const activeMovement = movements[activeMovementIdx];
    ...
    for (let i = 0; i < Math.abs(activeMovement); i++) {
        ...
        for (const ropeIndex in ropes) {
            ...
            for (const knotIndex in rope) {
                ...
                const distanceToPrevious = [rope[+knotIndex - 1][0] - knot[0], rope[+knotIndex - 1][1] - knot[1]];
