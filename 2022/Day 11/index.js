...
const monkeys = Array(input.length);
...
// Part 1
const monkeysBusiness1 = Array(input.length).fill(0);
for (let i = 0; i < 20; i++) {
    for (let j = 0; j < monkeysPart1.length; j++) {
        const monkey = monkeysPart1[j];
        monkeysBusiness1[j] += monkey.items.length;

        for (let k = 0; k < monkey.items.length; k++) {
            const item = monkey.items[k];
            const newItem = eval(`Math.floor((${item}${monkey.operation.replace("old", item)})/3)`);
...
const superModulo = monkeysPart2.reduce((a, b) => a * b.test.test, 1);
const monkeysBusiness2 = Array(input.length).fill(0);
for (let i = 0; i < 10000; i++) {
    for (let j = 0; j < monkeysPart2.length; j++) {
        const monkey = monkeysPart2[j];
        monkeysBusiness2[j] += monkey.items.length;

        for (let k = 0; k < monkey.items.length; k++) {
            const item = monkey.items[k] % superModulo;
            const newItem = eval(`${item}${monkey.operation.replace("old", item)}`);
