    ...
    for (let j = 0; j < boards.length; j++) {
        ...
        for (let k = 0; k < boards[j].length; k++) {
            if (boards[j][k].indexOf(numbers[i]) !== -1) {
                let numberIndex = boards[j][k].indexOf(numbers[i]);
                ...
                if (
                    ...
                    let score = boards[j].flat().filter(elem => elem !== -1).reduce((a, b) => a + b, 0) * numbers[i];
